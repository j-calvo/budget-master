...
import { 
  ...
  AlertCircle, RefreshCw, Plus, X, Eye
} from 'lucide-react';
...
export default function Services() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Build data payload and fallback parsing floats
      const payload = {
        ...formData,
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-serif text-white tracking-wide">{t('Utility Consumption')}</h1>
          <p className="text-sm font-medium text-slate-400 mt-1">{t('Track your household services and energy consumption')}</p>
        </div>
        <div className="flex gap-3">
          <button 
            onClick={() => fileInputRef.current?.click()}
            className="btn-glass px-4 py-2 text-xs flex items-center gap-1.5 shadow-sm active:scale-95 transition-all"
          ...
          <button 
            ...
            className="btn-gold px-5 py-2 text-xs font-semibold shadow-md flex items-center gap-1 active:scale-95 transition-all"
          >
      ...
      {isLoading ? (
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* 1. Monthly Consumption Trend (Bar) */}
            ...
            {/* 3. Cost & Spending Trend (Line) - full width */}
